...
class Bank(object):
    ...
    def transfer(self, origin, dest, amount):
        ...
        if not (hasattr(des, "value") and hasattr(ori, "value")):
            # raise TypeError("Corrupted account detected."))
            return False 
        if not (isinstance(des.value, int) or isinstance(des.value, float)):
            # raise TypeError("Bad destination value. Expected int or float. Got " + str(type(des.value)))
            return False
        if not (isinstance(ori.value, int) or isinstance(ori.value, float)):
            # raise TypeError("Bad origin value. Expected int or float. Got " + str(type(des.value)))
            return False
        if not (isinstance(amount, int) or isinstance(amount, float)):
            # raise TypeError("Expected int or float. Got " + str(type(amount)))
            return False
        if amount < 0:
            # raise ValueError("Requested mount below 0")
            return False
        if amount > ori.value:
            # raise ValueError("Requested amount above origin account balance")
            return False
        ori.value -= amount
        des.value += amount
        return True
...
SG = Bank()
test = Account("Hello", value="Hi", betail="h", Amount=3)
SG.add(test)